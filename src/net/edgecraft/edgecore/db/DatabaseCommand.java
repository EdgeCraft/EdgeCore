	...
	public boolean runImpl(Player player, User user, String[] args) throws Exception {
		...
		if ( args[1].equalsIgnoreCase("check") ) {				
			...
			if (args.length == 2) {
				...
				String connected = db.isAvailable() ? ChatColor.GREEN + "Verbunden!" : ChatColor.RED + "Nicht verbunden!";
				player.sendMessage(lang.getColoredMessage(userLang, "db_check").replace("[0]", connected));
		...
		if (args[1].equalsIgnoreCase("connect")) {
			...
			try {
					...
					// Try to load default connection
					db.loadConnection();
				}
		}
